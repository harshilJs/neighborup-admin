              ...
              <tr key={user.id} className="border-b border-gray-800 last:border-0 hover:bg-gray-800/50">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2.5">
                    <div className="w-7 h-7 rounded-full bg-gray-800 flex items-center justify-center text-xs text-gray-300 font-medium shrink-0">
                      {initials(user.display_name ?? user.full_name)}
                    </div>
                    <div>
                      <p className="text-white font-medium leading-tight">{user.display_name ?? user.full_name ?? 'Unnamed'}</p>
                      <p className="text-gray-500 text-xs">{user.email ?? '—'}</p>
                    </div>
                  </div>
                </td>
                <td className="px-4 py-3 text-gray-300">{TYPE_LABELS[user.account_type] ?? user.account_type}</td>
                <td className="px-4 py-3">
                  <StatusBadge
