...
export default async function Page() {
  ...
  return (
    <div>
      ...
      <p className="text-gray-500 text-sm mb-4">Showing the notification log. Composing and sending new campaigns isn&apos;t wired up yet — no push provider integration exists in this codebase.</p>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {notifications?.map(n => (
              <tr key={n.id} className="border-b border-gray-800 last:border-0 hover:bg-gray-800/50">
                <td className="px-4 py-3 text-gray-300">
                  {profileMap.get(n.user_id)?.full_name ?? profileMap.get(n.user_id)?.email ?? '—'}
                </td>
                <td className="px-4 py-3 text-gray-300">{n.type}</td>
                <td className="px-4 py-3 text-gray-300 max-w-xs truncate" title={n.content ?? undefined}>
                ...
                <td className="px-4 py-3 text-gray-400">{formatDateTime(n.created_at)}</td>
              </tr>
            ))}
          </tbody>
        </table>
